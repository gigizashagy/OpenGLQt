...
void main(void)
{
    vec4 wire_color = vec4(0.0,1.0,0.0,0.0);
    ...
    vec4 diffColor = diffMatColor * u_lightPower * max(0.0, dot(v_normal, -lightVect));// / (1.0 + 0.05 * pow(len, 2.f));
    resultColor += diffColor;
    vec4 ambientColor = ambientFactor * diffColor ;
    resultColor += ambientColor;
    vec4 specularColor = vec4(1.0,1.0,1.0,1.0) * u_lightPower * pow(max(0.0, dot(reflectLight, -eyeVect)), specularFactor) / (1.0 + 0.25 * pow(len, 2.f));
